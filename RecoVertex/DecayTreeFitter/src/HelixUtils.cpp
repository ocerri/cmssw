...
namespace DecayTreeFitter
{
  ...
  {
    double rc = phi ;
    if     (phi < -TMath::Pi())  rc += 2*TMath::Pi() ;
    else if(phi >  TMath::Pi())  rc -= 2*TMath::Pi() ;
    return rc ;
  }
  ...
  double VtkHelixUtils::helixPoca(const HepVector& helixpar1,
				  ...
				  GlobalPoint& vertex, bool parallel)
  {
...
//  	 << x0_2 << "," << y0_2 << "," << r_2 << endl ;
    ...
    vertex = GlobalPoint( 0.5*(x1+x2), 0.5*(y1+y2), 0.5*(z1+z2) ) ;
				  ...
				  double& flt)
  {
    ...
    for(int n = 1-ncirc; n <=1+ncirc ; ++n) {
      double l = (dphi + n*2*TMath::Pi())/omega ;
