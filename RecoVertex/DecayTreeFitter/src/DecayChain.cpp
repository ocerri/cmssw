...
namespace DecayTreeFitter
{
  ...
  DecayChain::DecayChain(const reco::Candidate& bc, const Configuration& config)
  ...
  DecayChain::DecayChain(const reco::Candidate& bc, const reco::Vertex& pv, const Configuration& config)
    : m_dim(0),m_mother(0),m_isOwner(true)
  {
  ...
  ErrCode DecayChain::filter(FitParams& par, bool firstpass) {
    ...
    if(m_mergedconstraintlist.empty() ) {
      for( ParticleBase::constraintlist::const_iterator it = m_constraintlist.begin(); it != m_constraintlist.end(); ++it) {
  ...
  const ParticleBase* DecayChain::locate(const reco::Candidate& bc) const {
  ...
  void DecayChain::setMassConstraint( const reco::Candidate& bc, bool add) {
  ...
  void DecayChain::setMassConstraint( const reco::Candidate& bc, double mass) {
    ParticleBase* part = const_cast<ParticleBase*>(locate(bc)) ;
    if(part) {
  ...
  void DecayChain::setMassConstraint( const reco::CandidateID& pid, bool add) {
  ...
  void DecayChain::setMassConstraint( const reco::CandidateID& pid , double mass ) {
    ParticleBase::ParticleContainer particles ;
    m_mother->locate( pid, particles ) ;
    bool changed(false) ;
    for(ParticleBase::ParticleContainer::iterator it = particles.begin() ;
  ...
  int DecayChain::index(const reco::Candidate& bc) const {
    int rc = -1 ;
  ...
  int DecayChain::posIndex(const reco::Candidate& bc) const {
    const ParticleBase* base = locate(bc) ;
    return base ? base->posIndex() : -1 ;
  }

  int DecayChain::momIndex(const reco::Candidate& bc) const {
    const ParticleBase* base = locate(bc) ;
    return base ? base->momIndex() : -1 ;
  }

  int DecayChain::lenIndex(const reco::Candidate& bc) const {
    const ParticleBase* base = locate(bc) ;
    return base ? base->lenIndex() : 0 ;
  }

  ChiSquare DecayChain::chiSquare( const reco::Candidate& bc, const FitParams& fitpars ) const {
    const ParticleBase* base = locate(bc) ;
    ChiSquare chisq ;
    if( base ) {
      ...
      if( base->mother() && base->posIndex()>=0 && base->posIndex() != base->mother()->posIndex() )	chisq += ChiSquare(0,1) ;
    }
    return chisq ;
  }
}
