...
namespace DecayTreeFitter
{
  ...
  InteractionPoint::InteractionPoint(const reco::Vertex& ipvertex,
    ...
    : ParticleBase("IP")
  {
    ...
    m_ipCov    = ipvertex.covariance() ;
    int ierr ;
    m_ipCovInv = m_ipCov.Inverse(ierr) ;
    if(vtxverbose>=2) {
      std::cout << "InteractionPoint: initial beam spot = (" << m_ipPos[0] << "," << m_ipPos[1] << "," << m_ipPos[2] << ")" <<std::endl ;
